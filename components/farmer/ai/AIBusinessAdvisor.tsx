    ...
    <div className="space-y-6">
      <div>
        <h2 className="text-lg font-bold text-white flex items-center gap-1.5">
          ...
          AI Farm Business & Capital Planner
        </h2>
        <p className="text-slate-400 text-xs mt-0.5">
          Assess your agricultural balance sheets, calculate credit loan limits, and plan risk mitigation.
        </p>
      </div>
      ...
      <form onSubmit={handleAdvise} className="space-y-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="space-y-1">
            <label className="text-xs text-slate-400 font-mono">Current Revenues (₹)</label>
            <input
              type="number"
              required
              value={totalRevenue}
        ...
        >
          {loading ? (
            <span>Computing credit scores and risk indexes...</span>
          ) : (
            <>
      ...
      {result && (
        <div className="mt-6 border-t border-white/5 pt-6 space-y-6">
          {/* Inner tab controls */}
          <div className="flex border-b border-white/5 gap-4 overflow-x-auto shrink-0 bg-white/[0.01]">
          ...
          {activeSubTab === "profit" && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono">
                ...
                <div className="bg-white/5 p-3 rounded-xl">
                  <p className="text-[10px] text-slate-500">EFFICIENCY INDEX GRADE</p>
                  <span className="inline-block mt-1 text-xs font-bold text-emerald-400 bg-emerald-500/10 px-2.5 py-0.5 rounded border border-emerald-500/20">
                    Grade {result.profit_analysis.efficiency_grade}
                  </span>
                </div>
                <div className="bg-white/5 p-3 rounded-xl">
          ...
          {activeSubTab === "loans" && (
            <div className="space-y-4">
              <div className="p-4 bg-white/5 border border-white/10 rounded-2xl flex justify-between items-center font-mono">
                <div>
                  <h4 className="text-xs font-bold text-white">Eligible Kisan Credit Loan Cap</h4>
                  <p className="text-slate-500 text-[10px] mt-0.5">Estimated limit using regional scale of finance benchmarks.</p>
                </div>
          ...
          {activeSubTab === "growth" && (
            <div className="space-y-3">
              <h4 className="text-xs font-bold text-slate-400 font-mono flex items-center gap-1">
                ...
                EXPANSION & CROP DIVERSIFICATION STRATEGIES
              </h4>
